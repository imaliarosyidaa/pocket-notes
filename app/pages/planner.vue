...
<template>
  <UDashboardPanel
    ...
    resizable
  >
    <UDashboardNavbar title="Pilih Tanggal" />
    <div class="p-4 flex flex-col h-full overflow-hidden">
      ...
      <div class="mt-6 flex-1 overflow-y-auto">
        <div class="mb-4">
          <h3 class="font-bold text-sm text-gray-500 flex items-center gap-2 uppercase tracking-wider">
        ...
        <div v-if="tasksOnSelectedDate.length > 0" class="space-y-2">
          ...
          >
            <div class="flex justify-between items-start">
              <span class="text-[10px] font-bold text-primary italic">{{ task.category || 'No Category' }}</span>
              <UButton 
  ...
  <UDashboardPanel id="form-panel">
    <UDashboardNavbar title="Tambah Tugas Baru" :toggle="false">
      <template #right>
        <UButton
          v-if="selectedDate"
          icon="i-lucide-save"
    ...
    <div v-if="selectedDate" class="flex-1 p-4 sm:p-8 overflow-y-auto max-w-2xl mx-auto w-full">
      <div class="mb-8 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-100 dark:border-gray-800 flex items-center gap-4">
        <div class="bg-primary text-white p-3 rounded-lg text-center min-w-[60px]">
          <span class="block text-xs uppercase">{{ format(toJSDate(selectedDate), 'MMM') }}</span>
          <span class="block text-xl font-bold leading-none">{{ format(toJSDate(selectedDate), 'dd') }}</span>
        </div>
        <div>
          <h4 class="font-bold text-lg">Deadline Tugas</h4>
          <p class="text-sm text-gray-500">{{ format(toJSDate(selectedDate), 'EEEE, dd MMMM yyyy') }}</p>
        </div>
      </div>

      <div class="space-y-6">
        <UFormField label="Nama Tugas" required>
          <UInput v-model="state.taskName" placeholder="Apa yang ingin dikerjakan?" size="lg" />
        </UFormField>
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <UFormField label="Sub-tugas (Child) Dari">
            <USelectMenu
    ...
    <div v-else class="flex-1 flex items-center justify-center">
      <div class="text-center opacity-30">
        ...
        <p>Pilih tanggal di kalender untuk menambah tugas</p>
      </div>
    </div>
  </UDashboardPanel>
</template>